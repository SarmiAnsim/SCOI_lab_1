...
namespace SCOI_lab_1
{
    ...
    class MyImage
    {
        ...
        public MyImage(Image image) { this.image = image; }
        ...
        public void Multiply(Image img)
        {
            ...
            for (int j = 0; j < basis_image.Height; ++j)
                for (int i = 0; i < basis_image.Width; ++i)
                {
                    ...
                    Color resultColor = Color.FromArgb(
                        Clamp(first.A + second.A),
                        Clamp((int)((first.R * ((float)first.A / 255)) * (int)(second.R * ((float)second.A / 255))/255)),
                        Clamp((int)((first.G * ((float)first.A / 255)) * (int)(second.G * ((float)second.A / 255))/255)),
                        Clamp((int)((first.B * ((float)first.A / 255)) * (int)(second.B * ((float)second.A / 255))/255)));
